...
int main(){
  ...
  err=config_init(&cfg);  
  err=ini_write(D,"none","hot-gauss","data/hot-gauss",dom,null_velocity,&par,&cfg);
  err=config_free(&cfg);  
  
  cout << "reading\n";
  err=ini_simple_read("hot-gauss.cfg",&D,&N,&x,&u,&S,&cfg);
  if(err!=0)
    return err;
  ...
  cout << "ploting\n";
  err=sph_dens(D,N,Npoints,xp,x,S,h,xl,xu,gauss_e2s,"gauss_e2s_plot.dat",&wrp);if(err!=0){ cout << err << endl; return err;}
